...
@inline function smallest(args::NTuple{N,F}) where {N, F <: BigFloat}
	@inbounds m = args[1]
	for i in 2:N
		@inbounds arg = args[i]
		m = arg < m ? arg : m
...
@inline function horner(x::F, coeffs::NTuple{N,F}) where {N, F <: BigFloat}
	@inbounds v = coeffs[1]
	for i in 2:N
...
@inline function horner2(x::F, y::F, coeffs::NTuple{N,F}) where {N, F <: BigFloat}
	@inbounds v = coeffs[1]
	w = v
	for i in 2:N
		@inbounds coeff = coeffs[i]
...
@inline function hornerd(x::F, coeffs::NTuple{N,F}) where {N, F <: BigFloat}
	@inbounds v = coeffs[1]
	d = zero(F)
	for i in 2:N
		@inbounds coeff = coeffs[i]
...
@inline function intervalhorner(low::F, high::F, coeffs::NTuple{N,F}) where {N, F <: BigFloat}
...
@inline function posintervalhorner(low::F, high::F, coeffs::NTuple{N,F}) where {N, F <: BigFloat}
	@inbounds colow = coeffs[1]
	cohigh = colow
	for i in 2:N
		@inbounds coeff = coeffs[i]
...
function allrealrootintervalnewtonregulafalsi(coeffs::NTuple{N,F}, res::Ptr{F}, doms::Ptr{NTuple{2,F}}) where {N, F <: BigFloat}
	if N == 1
		return 0
	elseif N == 2
...
coefi=NTuple{7,BigFloat}((BigFloat(a), BigFloat(b), BigFloat(c), BigFloat(d), BigFloat(e), BigFloat(f), BigFloat(g)))
#coefi=NTuple{7,BigFloat}((BigFloat(2.4855534831418154e12), BigFloat(1.2375936757103965e15), BigFloat(-1.1620693333574219e9), BigFloat(-1.21270658290325e12), BigFloat(-4.129153524641228e7), BigFloat(-80.08000016083042), BigFloat(-4.0000000000000105e-5)))
