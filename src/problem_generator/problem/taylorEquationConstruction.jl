...
function transformFSimplecase(ex::Union{Float64,Int64,Expr,Symbol})
    ...
    cachexpr = Expr(:ref, :cache, 1) # add cache[1] to createT(ex,....)
...
is_binary(expr, op) = expr isa Expr &&
                      ...
                      #!(expr.args[2] isa Number && expr.args[3] isa Number) &&  # Prevent number+number but the problem currently if rhs=number+number only then cache would be empty
