...
C = 3.08*1e-3#3.08*1e-3
          ...
          discrete = [1e5,1e-5,1.0,1e5,1e-5,1.0,0.0];u = [0.0,5990.75,0.0,0.0,9000.75,0.0]
          rd1=discrete[1];rs1=discrete[2];charge1=discrete[3];rd2=discrete[4];rs2=discrete[5];charge2=discrete[6];start=discrete[7]#LR = discrete[3];nextT=discrete[4];manualIntg=discrete[5]
          is1=u[1] ;uc1=u[2]; il1=u[3] ;is2=u[4] ;uc2=u[5]; il2=u[6] 
          #α=LR+Lpr;
          RR=4.0e-5
        
          du[1] =((-(R1+rs1+rd1)*is1+rd1*il1+uc1)/L1)*charge1*start
          du[2]=(-is1/C)*charge1*start
          du[3]=start*(((-rd1-R2-R3-RR)*il1-RR*il2+rd1*is1)/(L2+L3))#((L2+L3+α)*β)/Δ
   ...
   #=  save_Sol(sol,1)
    ...
    save_Sol(sol,5)
    save_Sol(sol,6) =#
    save_SolSum(sol,3,6) #add interpl preference
